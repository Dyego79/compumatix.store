...
async function seedProducts() {
  ...
  while (true) {
    ...
    await Promise.all(
      data.resultado.map(async (item: any) => {
        ...
        try {
          ...
          if (imagenes?.[0]) {
            try {
              newImageUrl = await subirImagenAUploadThing(imagenes[0]);
              console.log(`✅ Imagen subida para ${nombre}`);
            } catch (error) {
              console.error(
                `⚠️ Falló subir imagen de ${nombre}, usando original.`
              );
              newImageUrl = imagenes[0]; // fallback
            }
          }
    ...
    if (insertBatch.length > 0) {
      try {
        ...
        `;
        console.log(`✅ Insertados ${insertBatch.length} productos.`);
        totalInserted += insertBatch.length;
      } catch (error) {
        console.error(`❌ Error insertando batch:`, error);
      }
  ...
  console.log(
    `🏁 Seed finalizado. Total productos insertados: ${totalInserted}`
  );
}
